...
function TopNav() {
  ...
  return (
    <header className="bg-zinc-950 text-white sticky top-11 z-10">
      <div className="max-w-[1400px] mx-auto px-8 h-16 flex items-center gap-8">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-white text-zinc-950 text-[13px] font-extrabold flex items-center justify-center">
            N
          </div>
        ...
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <input
              placeholder="Search"
              className="w-64 bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-2 text-[13px] font-medium focus:outline-none focus:border-white/25 placeholder-zinc-500 text-white"
            />
          ...
          <button className="h-9 px-4 rounded-full bg-white text-zinc-950 text-[13px] font-bold hover:bg-zinc-200">
            + New
          </button>
        ...
        {["Today", "Week", "Month"].map((t, i) => (
          <button
            ...
            className={
              ...
              (i === 1 ? "bg-zinc-950 text-white" : "text-zinc-500 hover:text-zinc-950")
            }
          >
            {t}
          </button>
        ...
        <Card key={m.label}>
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[13px] text-zinc-500 font-semibold">{m.label}</div>
              <span
                className={
                  "text-[11px] px-2 py-0.5 rounded-full tabular-nums font-bold " +
                  (m.up ? "bg-zinc-950 text-white" : "bg-zinc-100 text-zinc-700 border border-zinc-200")
                }
              >
                {m.delta}
              </span>
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <section
      className={
        "bg-white rounded-2xl border border-zinc-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.03)] " +
        className
      }
    >
      {children}
...
function Area() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-44" preserveAspectRatio="none">
      ...
      <path d={fill} fill="url(#gGrad)" />
      <path d={path} stroke="#09090b" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function PipelineCard() {
  ...
  return (
    <Card>
      <CardHeader
        title="Pipeline"
        subtitle="35 active opportunities"
      />
  ...
  return (
    <Card className="col-span-2">
      <CardHeader
        ...
        action={
          <Link href="#" className="text-[12px] text-zinc-600 hover:text-zinc-950 font-bold">
            View all →
          </Link>
        }
      />
      <div className="p-3">
        {jobs.map((j, i) => (
          ...
          >
            ...
            <div className="text-[14px] font-bold tabular-nums w-16 text-right">
              {j.price}
            </div>
          </div>
        ))}
  ...
  return (
    <Card>
      <CardHeader
        title="Activity"
        action={
          <span className="text-[10px] text-zinc-700 flex items-center gap-1.5 font-bold tracking-wider">
            <span className="w-1.5 h-1.5 rounded-full bg-zinc-950" /> LIVE
          </span>
        }
      />
      <ul className="p-2">
        {items.map((it, i) => (
          <li key={i} className="flex items-start gap-3 px-3 py-2.5">
            <span className="inline-flex w-8 h-8 rounded-full bg-zinc-100 text-[11px] items-center justify-center font-bold text-zinc-700">
              {it.who[0]}
            </span>
            <div className="flex-1 text-[13px]">
              <span className="font-bold">{it.who}</span>{" "}
              <span className="text-zinc-600 font-medium">{it.what}</span>
              <div className="text-[11px] text-zinc-400 mt-0.5 tabular-nums font-bold">{it.when} ago</div>
            </div>
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="h-11 bg-white border-b border-zinc-200 flex items-center justify-between px-4 text-[12px] sticky top-0 z-20">
      ...
      <div className="font-bold tracking-tight text-zinc-950">
        Concept G · <span className="text-zinc-500">Balanced Light</span>
      </div>
