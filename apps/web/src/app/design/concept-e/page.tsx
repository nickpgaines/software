...
function Sidebar() {
  ...
  return (
    <aside className="bg-zinc-950 rounded-3xl p-5 flex flex-col sticky top-4 self-start h-[calc(100vh-72px)]">
      <div className="flex items-center gap-3 mb-7 px-1">
        <div className="w-10 h-10 rounded-2xl bg-white text-black text-[15px] font-extrabold flex items-center justify-center">
          N
        </div>
        <div className="flex-1">
          <div className="font-bold tracking-tight text-[16px]">Forge CRM</div>
        </div>
      </div>
      ...
      <div className="space-y-6 flex-1 overflow-auto">
        {sections.map((s) => (
          <div key={s.label}>
            <div className="text-[11px] uppercase tracking-[0.16em] text-zinc-600 px-2 mb-2 font-bold">
              {s.label}
            </div>
            <ul className="space-y-1">
              {s.items.map((it) => {
                ...
                return (
                  ...
                  >
                    <span className={"w-5 h-5 flex items-center justify-center " + (it.active ? "text-black" : "text-zinc-500")}>
                      <Icon />
                    </span>
                    ...
                    {it.count != null && (
                      <span
                        className={
                          ...
                          (it.active ? "bg-black/10 text-black" : "bg-zinc-900 text-zinc-300")
                        }
                      >
                        {it.count}
                      </span>
      ...
      <div className="mt-5 p-3 rounded-2xl bg-zinc-900 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-zinc-800 text-[13px] font-bold flex items-center justify-center">
          NG
        </div>
...
function Hero() {
  return (
    <div className="bg-zinc-950 rounded-3xl p-7 flex items-center justify-between gap-6 flex-wrap">
      <div>
        ...
        <p className="text-[15px] text-zinc-400 mt-1 font-medium">
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          <input
            ...
            className="w-72 bg-black border border-zinc-900 rounded-full pl-11 pr-4 py-3 text-[14px] font-medium focus:outline-none focus:border-zinc-700 placeholder-zinc-600"
          />
          <svg className="w-4 h-4 text-zinc-500 absolute left-4 top-1/2 -translate-y-1/2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>
        </div>
        <button className="h-12 px-6 rounded-full bg-white text-black text-[14px] font-bold hover:bg-zinc-200">
          + New job
        </button>
      </div>
    </div>
...
function KpiCards() {
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {k.map((m) => (
        <div key={m.label} className="bg-zinc-950 rounded-3xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="text-[14px] text-zinc-400 font-semibold">{m.label}</div>
            <div
              className={
                "text-[12px] px-2.5 py-1 rounded-full tabular-nums font-bold " +
                (m.up ? "bg-white text-black" : "bg-zinc-800 text-zinc-300")
              }
            >
              {m.delta}
            </div>
          ...
          <div className="text-[12px] text-zinc-500 mt-2 font-medium">vs last week</div>
        </div>
      ))}
    </div>
  );
...
function RevenueCard() {
  return (
    <section className="bg-zinc-950 rounded-3xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-[18px] font-bold tracking-tight">Revenue</h3>
          <p className="text-[13px] text-zinc-500 font-medium">Last 12 weeks</p>
        </div>
          ...
          {["12W", "26W", "YTD"].map((t, i) => (
            <button
              ...
              className={
                ...
                (i === 0 ? "bg-white text-black" : "text-zinc-500 hover:text-white")
              }
            >
              {t}
            </button>
...
function BigArea() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-44" preserveAspectRatio="none">
      ...
      <path d={path} stroke="#fff" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
    ...
    <section className="bg-zinc-950 rounded-3xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[13px] text-zinc-500 font-medium">{jobs.length} of 12 visible</p>
        </div>
        <Link href="#" className="text-[13px] text-zinc-400 font-bold hover:text-white">
          View all →
        </Link>
      </div>
      <div className="space-y-2">
        {jobs.map((j, i) => (
          <div
            key={i}
            className="flex items-center gap-4 p-4 rounded-2xl bg-black hover:bg-zinc-900 transition-colors cursor-pointer"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[13px] text-zinc-500 truncate font-medium">{j.addr}</div>
            </div>
            <span
              className={
...
function PipelineCard() {
  ...
  return (
    <section className="bg-zinc-950 rounded-3xl p-6">
      ...
      <p className="text-[13px] text-zinc-500 font-medium mb-5">35 active</p>
      <div className="space-y-4">
        {stages.map((s) => (
          <div key={s.name}>
            <div className="flex items-baseline justify-between mb-2">
              <span className="text-[14px] font-bold">{s.name}</span>
              <span className="text-[12px] text-zinc-500 tabular-nums font-bold">
                {s.count} · {s.value}
              </span>
            </div>
      ...
      <ul className="space-y-3">
        {items.map((it, i) => (
          <li key={i} className="flex items-start gap-3">
            <span className="inline-flex w-9 h-9 rounded-full bg-black text-[12px] items-center justify-center font-bold text-white">
              {it.who[0]}
            </span>
            <div className="flex-1 text-[13px]">
              <span className="font-bold">{it.who}</span>{" "}
              <span className="text-zinc-400 font-medium">{it.what}</span>
              <div className="text-[11px] text-zinc-600 mt-0.5 tabular-nums font-bold">{it.when} ago</div>
            </div>
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="h-11 bg-black border-b border-zinc-900 flex items-center justify-between px-4 text-[12px]">
      ...
      <div className="font-bold tracking-tight text-white">
        Concept E · <span className="text-zinc-500">Bold Dark</span>
      </div>
      <div className="flex items-center gap-1.5 text-zinc-600 font-bold">
        <Link href="/design/concept-a" className="hover:text-white">A</Link>
        <span>·</span>
        <Link href="/design/concept-b" className="hover:text-white">B</Link>
        <span>·</span>
        <Link href="/design/concept-c" className="hover:text-white">C</Link>
        <span>·</span>
        <Link href="/design/concept-d" className="hover:text-white">D</Link>
        <span>·</span>
        <Link href="/design/concept-e" className="hover:text-white">E</Link>
        <span>·</span>
        <Link href="/design/concept-f" className="hover:text-white">F</Link>
      </div>
    </div>
  );
}
