...
// "Customer with a line through it" — disqualified. Composed from the
// lucide UserRound node (head + shoulders) plus a diagonal slash so the
// small chips and the pin glyph read identically.
const CustomerSlash = createLucideIcon("customer-slash", [
  ["circle", { cx: "12", cy: "8", r: "5", key: "head" }],
  ["path", { d: "M20 21a8 8 0 0 0-16 0", key: "shoulders" }],
  ["line", { x1: "4", y1: "4", x2: "20", y2: "20", key: "slash" }],
]);
...
export const PIN_STATUS = {
  ...
  not_qualified:  { label: "Not Qualified",  color: "#a78bfa", textColor: "#ffffff", icon: CustomerSlash },
  do_not_contact: { label: "Do Not Contact", color: "#0f172a", textColor: "#ffffff", icon: Skull },
  revisit:        { label: "Revisit",        color: "#22d3ee", textColor: "#ffffff", icon: RotateCcw },
  referral:       { label: "Referral",       color: "#ec4899", textColor: "#ffffff", icon: Handshake },
...
export function filledGlyphSvg(status: PinStatus, color: string, size = 16): string {
  ...
  switch (status) {
    ...
    case "not_qualified": {
      // Customer (person) with a diagonal slash — disqualified. Line art
      // (matches the CustomerSlash chip icon), so stroke not fill.
      const s = `stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"`;
      return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24"><circle cx="12" cy="8" r="5" ${s}/><path d="M20 21a8 8 0 0 0-16 0" ${s}/><line x1="4" y1="4" x2="20" y2="20" ${s}/></svg>`;
    }
    case "do_not_contact":
    ...
    case "referral": {
      // Handshake (lucide). Line art, so stroke not fill.
      const s = `stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"`;
      return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24"><path d="m11 17 2 2a1 1 0 1 0 3-3" ${s}/><path d="m14 14 2.5 2.5a1 1 0 1 0 3-3l-3.88-3.88a3 3 0 0 0-4.24 0l-.88.88a1 1 0 1 1-3-3l2.81-2.81a5.79 5.79 0 0 1 7.06-.87l.47.28a2 2 0 0 0 1.42.25L21 4" ${s}/><path d="m21 3 1 11h-2" ${s}/><path d="M3 3 2 14l6.5 6.5a1 1 0 1 0 3-3" ${s}/><path d="M3 4h8" ${s}/></svg>`;
    }
    case "quote":
