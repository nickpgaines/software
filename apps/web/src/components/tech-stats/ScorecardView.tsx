...
export type Series = { date: string; cents: number };
...
export const PRESETS: { key: Exclude<Range, "custom">; label: string }[] = [
...
export function avgCents(series: Series[]) {
...
export type ScorecardViewProps = {
  staff: TechStats["staff"] | null | undefined;
  data: TechStats | null;
  range: Range;
  loading?: boolean;
  onRangeChange?: (r: Exclude<Range, "custom">) => void;
  onSalesClick?: () => void;
  // When true, suppresses the top row (back-link to /leaderboard + the
  // Sales/Tech navigation toggle). Used by the marketing site, where we
  // don't want to expose CRM-internal navigation affordances.
  hideNav?: boolean;
};
