...
type StatusFilterKey = "scheduled" | "in_progress" | "completed" | "cancelled";
...
function jobMatchesStatusFilter(job: Job, active: Set<StatusFilterKey>): boolean {
  if (active.size === 0) return true;
  if (job.status === "cancelled") return active.has("cancelled");
  if (job.job_status === "in_progress") return active.has("in_progress");
  if (job.job_status && job.job_status.startsWith("completed_"))
    return active.has("completed");
  return active.has("scheduled");
}
...
export default function CalendarClient() {
  ...
  const filteredJobs = useMemo(() => {
    if (!jobs) return null;
    if (statusFilter.size === 0) return jobs;
    return jobs.filter((j) => jobMatchesStatusFilter(j, statusFilter));
  }, [jobs, statusFilter]);
  ...
  return (
    ...
    <div className="relative h-[calc(100dvh-4.5rem-env(safe-area-inset-bottom))] md:h-[100dvh] flex flex-col overflow-hidden pt-[env(safe-area-inset-top)] md:pt-0">
      ...
      <div ref={gridRef} className="flex-1 min-h-0 overflow-auto bg-card">
        ...
        {view === "map" && (
          <MapView jobs={filteredJobs ?? []} rangeStart={range.start} rangeEnd={range.end} />
        )}
      </div>
...
function DayView({
  ...
  if (
    ...
    assigned = assigned.filter((s) => s.id === me.staff_id);
  }

  const unassignedTimed = timed.filter((j) => j.technician_id == null);
  const unassignedAnytime = anytime.filter((j) => j.technician_id == null);
  const hasUnassigned =
    unassignedTimed.length + unassignedAnytime.length > 0;

  type Lane = { staff: StaffLite | null; timed: Job[]; anytime: Job[] };
  const lanes: Lane[] = [];
  if (hasUnassigned) {
    lanes.push({
      staff: null,
      timed: unassignedTimed,
      anytime: unassignedAnytime,
    });
  }
  for (const s of assigned) {
    lanes.push({
      staff: s,
      timed: timed.filter((j) => j.technician_id === s.id),
      anytime: anytime.filter((j) => j.technician_id === s.id),
    });
  }
  // Empty day → render a single placeholder column so hover-to-add still
  // has a target. The placeholder has no header beyond the date.
  if (lanes.length === 0) {
    lanes.push({ staff: null, timed: [], anytime: [] });
  }
    ...
    <div className="md:min-w-[640px]">
      <div className="sticky top-0 z-30 bg-card">
        ...
        >
          <div />
          {lanes.map((lane, i) => {
            const totalCents = [...lane.timed, ...lane.anytime].reduce(
              (a, j) => a + (j.price_cents || 0),
              0
            );
            const count = lane.timed.length + lane.anytime.length;
            const isHere =
              lane.staff?.id != null && me?.staff_id === lane.staff.id;
            const hasHeader = !!lane.staff || hasUnassigned;
            if (!hasHeader) {
              return <div key={i} />;
            }
            return (
...
function MapView({
  ...
  const inRange = useMemo(() => {
    return jobs
      ...
      .filter((j) => !!(j.customer_address && j.customer_address.trim()))
      .sort(
        (a, b) =>
          new Date(a.scheduled_at).getTime() -
          new Date(b.scheduled_at).getTime()
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="text-sm font-bold text-white">Jobs in this range</div>
          <div className="text-xs text-zinc-500 mt-0.5">
            {inRange.length} with addresses · tap to open in Maps
          </div>
        </div>
        <Link
          href="/map"
          className="inline-flex items-center gap-1.5 border border-line bg-card hover:bg-black rounded-full px-4 py-2 text-sm text-zinc-300 font-bold"
        >
          <svg
            className="w-4 h-4"
            viewBox="0 0 24 24"
      ...
      {inRange.length === 0 ? (
        <div className="rounded-2xl border border-line bg-card p-6 text-center">
          <div className="text-sm font-bold text-zinc-200">No jobs to show</div>
          <div className="text-xs text-zinc-500 mt-1">
            No scheduled jobs with addresses in this range.
          </div>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <a
                  ...
                  className="shrink-0 inline-flex items-center gap-1 border border-line hover:bg-black rounded-full px-3 py-1.5 text-xs text-zinc-300 font-bold"
                >
                  <svg
...
// from DAY_START_HOUR.
// Default duration for a new job placed via the hover-to-add bubble.
// Most window-cleaning visits run two hours, so that's a better default
// than the 1-hour Calendar/Outlook standard.
const DEFAULT_NEW_JOB_MIN = 120;

function useHoverSlot(frozen = false) {
...
function HoverAddPopover({
  ...
  const endLabel = timeLabel(end.toISOString());
  const dateLabel = day.toLocaleDateString(undefined, {
    month: "long",
    day: "numeric",
  });
    ...
    >
      ...
      <DropdownMenu open={menuOpen} onOpenChange={onMenuOpenChange}>
        ...
        <DropdownMenuContent align="start" sideOffset={6} className="w-56">
          ...
          <DropdownMenuSeparator />
          <div className="px-2 py-1.5 text-center text-xs text-zinc-400">
            {startLabel} – {endLabel}
          </div>
        </DropdownMenuContent>
      </DropdownMenu>
